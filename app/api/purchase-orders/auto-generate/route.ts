...
export async function POST() {
  ...
  const lowStockRecords = await prisma.inventoryRecord.findMany({
    where: {
      ...
      quantity: { lte: prisma.inventoryRecord.fields.reorderPoint },
    },
    include: {
      item: { include: { purchaseLines: { include: { purchaseOrder: { include: { supplier: true } } }, orderBy: { purchaseOrder: { createdAt: "desc" } }, take: 1 } } },
    },
  });
  ...
  // Group by most recently used supplier (or first available)
  ...
  // Create a single draft PO with all low-stock items
