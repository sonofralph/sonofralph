      ...
      <div className="flex w-full lg:w-1/2 flex-col items-center justify-center px-8 py-12">
        <div className="w-full max-w-sm space-y-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button type="submit" className="w-full h-11 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold shadow-sm" disabled={isLoading}>
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-2">
            ...
            {[
              ...
              <button key={demo.email} type="button" onClick={() => { setEmail(demo.email); setPassword("password123"); }}
                className={cn("flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm transition-colors", "hover:bg-white hover:shadow-sm border border-transparent hover:border-slate-200")}>
          ...
          <p className="text-center text-xs text-slate-400">
            Need an account?{" "}
            <Link href="/register" className="font-medium text-indigo-600 hover:underline">Create organization</Link>
          </p>
        </div>
      </div>
    </div>
